...
$createStartupScript = Read-Host -Prompt "Do you want to create a script that you'll manually run to set up the SSH key at Windows startup? (Y/N)"
...
"@
    ...
    Write-Host "You can manually run this script at Windows startup."
    Write-Host ""
} else {
...
# Ask if user wants to add an alias for the script in PowerShell profile
...
if ($addToProfile -eq 'Y' -or $addToProfile -eq 'y') {
    ...
    if (-not (Select-String -Path $profilePath -Pattern 'Set-Alias auth')) {
        ...
        Write-Host "At Windows startup, in the terminal, you'll need to type 'auth' to run the authentication script" -ForegroundColor Blue
